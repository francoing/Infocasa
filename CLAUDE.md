...
6. Antes de finalizar, los **gates duros** (todos corren en CI): `npm run lint` (fitness function) · `npm run knip` (código muerto) · `npm run map:check` (exactitud del PROJECT-MAP) · `npm run test` (Vitest sobre happy-dom). **Los tests son gate:** una feature/fix con lógica nueva suma o ajusta su test; no se baja el gate para "pasar".
