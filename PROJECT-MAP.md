...
- **Tamaño/complejidad (componentes/hooks gigantes)** — `PropertyForm` (1068), `DashboardPage` (981), `HomePage` (524), `useDashboardData`, `useProperties`, `usePropertyDetail`. *(✅ salieron enteros: `PropertyDetailPage` → `components/detail/` (spec `property/detail_split`); `ProfilePage` (591→200) → `components/` (avatar, planes, 3 forms) + lógica de MP en `useMercadoPagoReturn` (spec `profile/profile_split`).)*
