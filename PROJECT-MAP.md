...
- **Tamaño/complejidad (componentes/hooks gigantes)** — `HomePage` (524), `useDashboardData`, `useProperties`, `usePropertyDetail`. *(✅ salieron enteros: `PropertyDetailPage` → `components/detail/` (spec `property/detail_split`); `ProfilePage` (591→200) → `components/` + `useMercadoPagoReturn` (spec `profile/profile_split`); `DashboardPage` (981→204) → `components/` (spec `dashboard/dashboard_split`); `PropertyForm` (1068→95) → `usePropertyForm` + `propertyForm.helpers` + `components/form/` (spec `property/property_form_split`).)*
